...
*
* Program musi osetrovat pocet a spravnost argumentov. Program musi mat
help,
...
* Error: 'adresar, subor, program,... pri ktorom nastala chyba': popis
chyby ...
...
/*      prepinac s jednym parametrom
                       case 'p':
                               printf("Output: '%s': parameter prepinaca
-p\n", optarg);
                               exit(0);
*/
                       case '?':
                       default:
                               fprintf(stderr, "Error: '%c': nespravny
prepinac\n", optopt);
                               exit(1);
               }
       argc -= optind;
       argv += optind;
       if(argc < 3) {
               fprintf(stderr, "Error: zly pocet argumetnov (vid.
help)\n");
               exit(1);
       }
       ...
       if((argumenty = (char **) malloc((sizeof(char *) * argc) +
1))==NULL) {
               fprintf(stderr, "Error: zlyhalo volanie malloc
(pravdepodobne malo pamate)\n");
               exit(1);
       }
       ...
       if((fw=open(file, O_RDWR | O_CREAT | O_TRUNC, 0755))==-1) {
               fprintf(stderr, "Error: '%s': nepodarilo sa otvorit subor na
zapis\n");
               exit(1);
       }
       while(1) {
               ...
               switch(fork()) {
                       case -1:
                               fprintf(stderr, "Error: zlyhalo volanie
fork\n");
                               exit(1);
                       case 0:
                               close(pajpa[0]);
...
execvp\n");
                       ...
                       default:
                               close(pajpa[1]);
                               while((buffsize=read(pajpa[0], buffer,
80))!=0) {
